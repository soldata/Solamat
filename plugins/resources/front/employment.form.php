...
if (isset($_POST["add"])) {
   $employment->check(-1, 'w');
   $newID = $employment->add($_POST);

   Html::back();

} else if (isset($_POST["update"])) {
   $employment->check($_POST["id"], 'w');
   ...
   if (!empty($_POST['itemtype'])) {
      ...
      $employment->check($input["id"], 'w');
      $employment->update($input);
   }
   Html::back();

}else {
   $employment->checkGlobal("r");
   Html::header($LANG['plugin_resources']['title'][1],'',"plugins","resources","employment");

   $employment->showForm($_GET["id"],
