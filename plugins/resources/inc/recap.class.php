...
//         foreach ($searchopt["PluginResourcesRecap"] as $key => $val) {
      ...
      if ($result = $DB->query($query)) {
         ...
         if ($p['start']<$numrows) {
            ...
            if ($output_type==HTML_OUTPUT) {
               Html::printPager($p['start'],$numrows,$target,$parameters,"PluginResourcesRecap");
            }
...
//            }
            ...
            if ($output_type==HTML_OUTPUT) { // HTML display - massive modif
               ...
               if (Session::haveRight("search_config","w") || Session::haveRight("search_config_global","w")) {
                  $tmp = " class='pointer' onClick=\"var w = window.open('".$CFG_GLPI["root_doc"].
                     "/front/popup.php?popup=search_config&amp;itemtype=PluginResourcesRecap' ,'glpipopup', ".
            ...
            foreach ($toview as $key => $val) {
               ...
               echo Search::showHeaderItem($output_type,$searchopt["PluginResourcesRecap"][$val]["name"],
...
//                     }
//                  }
                  echo Search::showItem($output_type, '', $item_num, $row_num, "width='10'");
//               }
            ...
            if ($output_type==PDF_OUTPUT_PORTRAIT|| $output_type==PDF_OUTPUT_LANDSCAPE) {
               $title.=$LANG['plugin_resources']['helpdesk'][29];
            }
...
//                  Dropdown::showForMassiveAction("PluginResourcesRecap", $p['is_deleted']);
