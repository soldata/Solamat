...
function update120to130() {
   ...
   if (FieldExists('glpi_plugin_projet_tasks','realtime')) {
      ...
      $DB->query($query)
      or die("0.83 ADD actiontime in glpi_plugin_projet_tasks". $LANG['update'][90] . $DB->error());
      
      $query = "UPDATE `glpi_plugin_projet_tasks`
                SET `actiontime` = ROUND(realtime * 3600)";
      $DB->query($query)
      or die("0.83 compute actiontime value in glpi_plugin_projet_tasks". $LANG['update'][90] . $DB->error());

      $query="ALTER TABLE `glpi_plugin_projet_tasks`
         DROP `realtime` ;";
      $DB->query($query)
      or die("0.83 DROP realtime in glpi_plugin_projet_tasks". $LANG['update'][90] . $DB->error());
   }
   ...
   $DB->query($query)
   or die("0.83 Create glpi_plugin_projet_projets_projets". $LANG['update'][90] . $DB->error());
   ...
   if (!empty($projets)) {
      foreach ($projets as $projet) {
         ...
         $DB->query($query)
         or die("0.83 compute values in glpi_plugin_projet_projets_projets". $LANG['update'][90] . $DB->error());
      }
   }
   ...
   $DB->query($query)
   or die("0.83 DROP plugin_projet_projets_id in glpi_plugin_projet_projets". $LANG['update'][90] . $DB->error());
   ...
   $DB->query($query)
   or die("0.83 Create glpi_plugin_projet_tasks_tasks". $LANG['update'][90] . $DB->error());
   ...
   if (!empty($tasks)) {
      foreach ($tasks as $task) {
         ...
         $DB->query($query)
         or die("0.83 compute values in glpi_plugin_projet_tasks_tasks". $LANG['update'][90] . $DB->error());
      }
   }
   ...
   $DB->query($query)
   or die("0.83 DROP plugin_projet_tasks_id in glpi_plugin_projet_tasks". $LANG['update'][90] . $DB->error());
