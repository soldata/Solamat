               ...
               foreach ($datas as $data) {
                  if (isset($data['id'])) {
                     if (isset($ressource->fields['locations_id']) 
                        ...
                        //$_POST["documents_id"] = PluginPositionsPosition::getDocument($ressource->fields['locations_id']);
                        $id            = $data['id'];
...
Html::popHeader($LANG['plugin_positions']['title'][1], $_SERVER['PHP_SELF']);
...
if (isset($locations_id) && !empty($locations_id)) {
   ...
   echo "<span class='b'>" . $LANG['plugin_positions'][25] . "</span></div>";
   //Html::nullFooter();
}
