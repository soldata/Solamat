...
if ($_SESSION['glpiactiveprofile']['interface'] == 'central') {
   ...
   //from helpdesk
   Html::helpHeader($LANG['plugin_positions']['title'][1]);
}

$PluginPositionsPosition=new PluginPositionsPosition();

if ($PluginPositionsPosition->canView() || Session::haveRight("config","w")) {
      
      if(!$_POST["locations_id"]) {
          //Html::back();
          PluginPositionsPosition::showLocationForm($_POST["locations_id"]);
          Html::displayErrorAndDie($LANG['plugin_positions']['setup'][8], false, ERROR);
