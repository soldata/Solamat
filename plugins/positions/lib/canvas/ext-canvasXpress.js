...
Ext.canvasXpress = Ext.extend(Ext.Panel, {
  ...
  afterRender: function() {
    ...
    //Ext.MessageBox.alert(this.data.toSource());
    this.canvas = new CanvasXpress(this.canvasId, this.data, this.options, this.events);
    this.canvas.Ext = this;
    this.canvas.highlightNode = this.highlightArray; // clear up the initial highlight

    //alert(this.highlightArray.x + ' ' + this.highlightArray.y +' '+this.highlightArray.width +' ' + this.highlightArray.height +' '+ this.canvas.nodeHighlightColor);
    this.canvas.draw();

    if(this.highlightArray.length>0){
        this.canvas.flashNode(this.highlightArray) ;
    }
  ...
  onContextMenu: function (e, o, r) {
    ...
    items: this.createContextMenu(e)
});
  ...
  networkMenu: function(o, type, it) {
    ...
    var items = [];
    if(type == 'cs' ) // right click menu only && droit d'écriture
    {
      //emgi -----> test sur les droits
      if(o && o.nodes && o.nodes.length && (o.nodes[0].testEvent == false)) // canvasXpress only sends 1 node over
      {
        ...
        if(o.nodes.length > 1)
        {
          ...
          items.push(/*{
            ...
            text: 'Editer les objets sélectionnés',
            ...
            text: 'Cacher les objets sélectionnés',
            handler: this.toggleNode.createDelegate(this, [n, true])
          }, {
            icon: this.imgDir + 'cancel.png',
            text: 'Cacher tous les autres objets',
          ...
          items.push(
           {
              ...
              handler: this.creationfenetre.createDelegate(this,[n])
           },
          {
            icon: this.imgDir + 'edit.png',
            text: 'Editer ' + label,
        ...
        //emgi --> si on a les droits de lecture seulement, on n'affiche qu'un onglet 'information'
        if(o && o.nodes && o.nodes.length && (o.nodes[0].testEvent == true)) // canvasXpress only sends 1 node over
        {
            if(this.canvas.selectNode)
            {
                var cnt = 0, found = false;
            ...
            if(o.nodes.length > 1)
            {
                ...
                items.push(
                    {
                        icon: this.imgDir + 'information.png',
                        text: 'Informations sur les éléments sélectionnés',
                        handler: this.creationfenetre.createDelegate(this,[n])
                    }
                );
                Ext.canvasXpress.utils.addMenu(items, n, this.nodeMenu, this);
      ...
      else
      {
        ...
        if(this.canvas.canNetworkRedoOp())
          items.push({
            text: 'Rejouer la dernière modification',
   ...
   creationfenetre: function(n){
       //name,id,label
       //var w = new Ext.canvasXpress.infoDialog(o, this);
       //w.show();
       if(typeof(n) != 'object' || !n.length) n = [n];
       for(var i = 0; i < n.length; i++){
           var win = 0;
           //emgi =>>>>> fonction qui permet de d'afficher les infos dans une fenêtre modale
           win = new Ext.Window({id:n[i].id,title:'Informations - '+n[i].name,width:'600px',height:'200px',
               modal:false,layout:'fit',resizable:false, autoLoad: {url: 'showinfos.php?items_id='+n[i].items_id+'&id='+n[i].id+'&name='+n[i].name+'&img='+n[i].imagePath+'&itemtype='+n[i].itemtype}
           });
           win.show();
           if( n[i].itemtype =='Location'){
               win.hide();
           }
       }
    },
  ...
  createContextMenu: function (e) {
    ...
    this.addItemToMenu(items, 'Général', false, this.generalMenus());
    ...
    items.push({
      ...
      text: 'Rafraichir le graphique'/* + this.canvas.graphType*/,
      scope: this.canvas,
    ...
    {
      items.push('-');
      items.push({text: 'Voir en tant qu\'image',
  ...
  axesMenus: function () {
    ...
    var axes = this.canvas.getValidAxes();
    if (axes) {
      this.addItemToMenu(items, 'Tailles de polices', false, this.axesFontSizes());
      this.addItemToMenu(items, 'Couleurs', false, this.axesColors(axes));
      ...
      this.addItemToMenu(items, 'Titres', false, this.axesTitles(axes));
  ...
  generalMenus: function () {
    var items = [];
    this.addItemToMenu(items, 'Couleurs', false, this.generalColors());
    //this.addItemToMenu(items, 'Dimensions', false, this.dimensions());
    this.addItemToMenu(items, 'Police', false, this.font());
    this.addItemToMenu(items, 'Layout', false, this.graphLayout());
    this.addItemToMenu(items, 'Titre', false, this.titles());
  ...
  background: function () {
    ...
    if (this.canvas.backgroundType == 'dégradé') {
      m.push({text: 'Couleurs',
	      iconCls: '',
  ...
  backgroundType: function () {
    return this.buildGroupMenu('backgroundType', ['solid', 'dégradé']);
  },
  ...
  simpleSearch: function() {
  {
    var win = new Ext.Window({
      title: 'Recherche sur les objets',
      width: 550,
      height: 300,
      x: 50,
      layout: 'anchor',
...
//           height: 35,
          items: [{
            ...
            items: [{
              xtype: 'textfield',
              emptyText: 'Le RegEx est accepté',
              enableKeyEvents: true,
              width: 200,
              listeners: {
                scope: this,
          ...
          cm: new Ext.grid.ColumnModel({
              ...
              columns: [
                ...
                { header: "Champ cherché", dataIndex:'name', width: 120 },
                { header: "Valeur trouvée", dataIndex:'detail', id: 'detail' }
            ]})
        }],
      bbar: {
        items: [
          {
            text: 'Arrêter la surbrillance',
            scope: this,
            handler : function(b) {
          ...
          '->',
          {
            text: 'Mettre en surbrillance l\'objet',
            scope: this,
            handler : function(b) {
          ...
          {
            text: 'Editer l\'objet sélectionné',
            scope: this,
            handler : function(b) {
              var grid = win.items.items[1], rows = grid.getSelectionModel().getSelections();
              if(rows && rows.length)
  ...
  deleteNode: function(n) {
    if(typeof(n) != 'object' || !n.length) n = [n];
    var str1 = n.length > 1? 'les objets sélectionnés ' : 'cet objet ',
        str2 = n.length > 1? 'les objets sélectionnés et tous les' : 'cet objet et tout ses';
    Ext.Msg.confirm('Attention', 'Etes-vous sur de vouloir supprimer '+str1+'? '+str2+' liens vont être supprimés !', function(b) {
      if(b == 'yes')
      {
        this.fireEvent('removenode', n, function() {
          for(var i = 0; i < n.length; i++){
            this.canvas.removeNode(n[i]);
            //emgi =>>>>> fonction qui permet de supprimer dans la base de données l'élément selectionné
            ...
            Ext.Ajax.request({
               ...
               success: function(response, opts) {
                  ...
                  Ext.select('#locationform input[name=_glpi_csrf_token]')
                     .set({'value': token});
                  Ext.select('#addfromplugin input[name=_glpi_csrf_token]')
                     .set({'value': token});
                  Ext.select('#massiveaction_form input[name=_glpi_csrf_token]')
                     .set({'value': token});
                  Ext.select('#pointform input[name=_glpi_csrf_token]')
                     .set({'value': token});
               },
              ...
              var tokentest = document.getElementsByName("_glpi_csrf_token").item(4).value;
              Ext.Ajax.request({
                  url: 'position.form.php',
                  method: 'POST',
                  params: {
                      deletepos: "deletepos",
                      id: n[i].id,
                      locations_id: n[i].locations_id,
                      _glpi_csrf_token:tokentest
                  }
              });
          }
          this.canvas.draw();
...
Ext.canvasXpress.nodeDialog = Ext.extend(Ext.Window, {
  constructor: function(config, extCanvas) {
    ...
    var item1 = [
      // label is required, parent, name, color, size, shape can all be hidden if user wants
      {
        ...
        items: [
          ...
          { xtype: 'displayfield', value: 'Taille:', width: 26, style:'margin-top:3px' },
      ...
      }/*,
      {
        ...
        emptyText: 'Sélection non obligatoire',
        triggerAction: 'all',
        name: 'parentLabel',
        mode: 'local',
        store: new Ext.data.ArrayStore({
    ...
    var item2 = [
      { xtype: 'colorfield', fieldLabel: 'Couleur', allowBlank: isMulti,
        ...
        hideLabel: h.color, hidden: h.color },
      {
        ...
        items: [
          ...
          { xtype: 'displayfield', value: 'Largeur:', width: 37, style:'margin-top:3px' },
      ...
      },
      {
        ...
        items: [
          ...
          { xtype: 'displayfield', value: 'Rotation:', width: 52, style:'margin-top:3px;margin-left:5px;' },
      ...
      },
      {
        ...
        fieldLabel: 'Taille',
        items: [
          { xtype:'numberfield', name:'Taille', hideLabel: h.size, hidden: h.size,
            width: 30, value: isMulti? null : config.size || '1.0' },
          { xtype: 'displayfield', value: 'Largeur:', width: 35, style:'margin-top:3px' },
          { xtype:'numberfield', name:'width', hideLabel: h.width, hidden: h.width,
            width: 30, value: isMulti? null : config.width || '' },
          { xtype: 'displayfield', value: 'Hauteur:', width: 40, style:'margin-top:3px' },
      ...
      {
        xtype: 'combo',
        fieldLabel: 'Type d\'objet',
        allowBlank: isMulti,
        hideLabel: h.shape, hidden: h.shape,
        emptyText: 'Sélection obligatoire',
        triggerAction: 'all',
      ...
      { fieldLabel: 'Chemin vers l\'image', value: isMulti? null : config.imagePath || '',
    ...
    bitems.push({
      ...
      '|',
      {
        text: isMulti? 'Modifier les objets' : (config.id? 'Modifier' : 'Ajout') + ' l\'objet',
        handler: bf
      });
    Ext.apply(this, {
      width: nd.width || 400,
      height: nd.height || 400,
      title: 'Editeur d\'objet' + (config.id? ' (Objet Id:' + config.id + ')':''),
      items: {
        xtype: 'tabpanel',
        activeItem: 0,
        defaultType: 'form',
