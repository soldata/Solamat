	...
	if (in_array($type, PluginImmobilizationsheetsItem::getTypes())) {
			if ($immo->getFromDB(1))
				if ($immo->fields["use_backup"]==1)	
					return array("plugin_immobilizationsheets_generate"=>$LANG['plugin_immobilizationsheets']['profile'][3]);
		}
	return array();
}
				...
				if ($val==1) {
					...
					if ($item->getFromDB($key)) {
						// Entity security
							...
							echo "<script type='text/javascript'>location.href='../plugins/immobilizationsheets/front/massiveexport.php'</script>)";
					}
				}
			}	
         break;	
