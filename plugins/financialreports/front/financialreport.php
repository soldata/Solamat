...
Html::header($LANG['plugin_financialreports']['title'][1],'',"utils","report");
...
if (isset($_POST["add_date"])) {
...
} else if ($report->canView() || Session::haveRight("config","w")) {
   ...
   if(!isset($_POST["locations_id"])) $_POST["locations_id"]= $_GET["locations_id"];
	...
	echo "<td class='right'>".$LANG['plugin_financialreports']['reports'][16]." :</td>";
	echo "<td>";
	...
	echo "</td>";
	echo "<td rowspan='2' class='center'><input type=\"submit\" class='button' name=\"choice_date\" Value=\"". $LANG['buttons'][2] ."\" /></td></tr>";
	echo "</table>";
	Html::closeForm();
	echo "</div>";
