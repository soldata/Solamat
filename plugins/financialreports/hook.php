...
function plugin_financialreports_install() {
   ...
   $update=false;
	if (!TableExists("glpi_plugin_state_profiles") 
            && !TableExists("glpi_plugin_financialreports_profiles")) {
      
      $DB->runFile(GLPI_ROOT ."/plugins/financialreports/sql/empty-1.7.0.sql");
	...
	} else if (!TableExists("glpi_plugin_financialreports_profiles")) {
...
function plugin_financialreports_uninstall() {
	...
	$tables = array("glpi_plugin_financialreports_profiles",
					"glpi_plugin_financialreports_configs",
   ...
   //old versions	
   $tables = array("glpi_plugin_state_profiles",
...
function plugin_financialreports_getDatabaseRelations() {
	...
	if ($plugin->isActivated("financialreports"))
		return array (
			...
			"glpi_profiles" => array ("glpi_plugin_financialreports_profiles" => "profiles_id")
		);
	else
		return array ();
}
...
function plugin_financialreports_getAddSearchOptions($itemtype) {
   ...
   if (in_array($itemtype, PluginFinancialreportsDisposalItem::getTypes())) {
      if (plugin_financialreports_haveRight("financialreports","r")) {
         ...
         $sopt[3450]['linkfield']='';
         $sopt[3450]['name']=$LANG['plugin_financialreports']['title'][1]." - ".
                              $LANG['plugin_financialreports']['reports'][17];
...
function plugin_financialreports_MassiveActions($type) {
	...
	if (in_array($type, PluginFinancialreportsDisposalItem::getTypes())) {
			return array("plugin_financialreports_add_date_disposal"
                        =>$LANG['plugin_financialreports']['reports'][11]);
		}
	return array();
}

function plugin_financialreports_MassiveActionsDisplay($options=array()) {
	global $LANG;
	
	if (in_array($options['itemtype'], PluginFinancialreportsDisposalItem::getTypes())) {
         ...
         foreach ($data["item"] as $key => $val) {
            if ($val==1) {
               ...
               if ($item->getFromDB($key)) {
                  ...
                  $disposal->addDateDisposal($key,$data['itemtype'],$data['date_disposal']);
