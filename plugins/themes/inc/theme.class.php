...
class PluginThemesTheme extends CommonDBTM {
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate=0) {
      ...
      if ($item->getType()=='Preference') {
         return $LANG['plugin_themes']['title'];
      }
      return '';
   }
   ...
   function showPluginFromPreference($item) {
      ...
      echo "<tr>";
      echo "<td>".$LANG['plugin_themes']['select'];
      Dropdown::show('PluginThemesTheme', array('name' => "plugin_themes_themes_id",
                                                'value' => $userCurrentThemeId));
      echo "</td><td>";
      echo "<input type='submit' class='submit' value='".$LANG['plugin_themes']['submit']."' />";
      echo "</td>";
      echo "<td>";
      echo "</tr>";
      echo "</table>";
   ...
   static function showAllThemes() {
      ...
      echo '<table class="tab_glpi" style="margin-bottom:20px;">
               <tr>
                  ...
                  <td class="icon_consol">
                     <span class="b">'.$LANG['plugin_themes']['warning'].'</span>
                  </td>
               </tr>
            </tbody></table>';
      ...
      echo "<tr>
               <th colspan='6'>".$LANG['plugin_themes']['allThemes']."</th>
            </tr>";
            
      echo "<tr>
               <th>".$LANG['plugin_themes']['name']."</th>
               <th>".$LANG['plugin_themes']['version']."</th>
               <th>".$LANG['plugin_themes']['author']."</th>
               <th>".$LANG['plugin_themes']['js']."</th>
               <th>".$LANG['plugin_themes']['preview']."</th>";
      ...
      foreach($allThemes as $themeDetails) {
         ...
         if(empty($themeDetails['url'])) {
            ...
            echo "<a title='".$LANG['plugin_themes']['goWebsite'].
         ...
         if(empty($themeDetails['mail'])) {
            ...
            echo "<a title='".$LANG['plugin_themes']['contact'].
         ...
         echo "<td class='center'>";
         if($themeDetails['js']) {
            echo $LANG['plugin_themes']['yes'];
         } else {
            echo $LANG['plugin_themes']['no'];
         }
         echo "</td>";
         ...
         if($themeDetails['name'] == "GLPI") {
            $defaultThemePath = GLPI_ROOT.'/plugins/themes/img/defaultscreenshot.jpg';
            $screenshotPath = $screenshotFile = $defaultThemePath;
         } else {
            $screenshotFile = PLUGIN_THEMES_UPLOAD_DIR.'/'.
               $themeDetails['name'].'/screenshot.jpg';

            $screenshotPath = GLPI_ROOT."/plugins/themes/front/getfile.php?theme_id=".
         ...
         if(is_file($screenshotFile)) {
            ...
            echo $LANG['plugin_themes']['noPreview'];
         }
         echo "</td>";
         ...
         if($t->canCreate()) {
            ...
            if($themeDetails['active_theme'] == 0) {
               echo "<a href='?activate=".$themeDetails['id']."'>".
                    $LANG['plugin_themes']['makeDefault']."</a><br /><br />";
            }
            if($themeDetails['name'] != "GLPI") {
               echo "<a href='?delete=".$themeDetails['id']."'>".
                  $LANG['plugin_themes']['delete']."</a>";
            }
            echo "</td>";               
         }
      ...
      echo $LANG['plugin_themes']['new_theme'];
      echo "</th></tr><tr><td class='center'>";

      echo "<input type='file' name='themeFile' />&nbsp;";
      echo "<input type='submit' class='submit' name='add_theme' value='".$LANG['plugin_themes']['submit']."' />";
