...
function plugin_accounts_install() {
   ...
   $update171=false;
	if (!TableExists("glpi_plugin_compte") 
      ...
      $install=true;
		$DB->runFile(GLPI_ROOT ."/plugins/accounts/sql/empty-1.8.0.sql");
   ...
   if ($update78) {
	...
	if (isset($_SESSION['plugin_acounts_upgrading'])) {
      $msg = "After plugin installation, you must do upgrade of your passwords from here : ";
      $msg .= "<a href='".$CFG_GLPI["root_doc"]."/plugins/accounts/front/upgrade.form.php'>upgrading Page</a>";
...
function plugin_accounts_AssignToTicket($types) {
	...
	if (plugin_accounts_haveRight("open_ticket","1"))
		$types['PluginAccountsAccount']=$LANG['plugin_accounts']['title'][1];
	return $types;
}
...
function plugin_accounts_getDatabaseRelations() {
	...
	if ($plugin->isActivated("accounts"))
		return array (
			...
			"glpi_profiles" => array (
				"glpi_plugin_accounts_profiles" => "profiles_id"
			)
		);
	else
		return array ();
...
function plugin_accounts_getDropdown() {
	...
	if ($plugin->isActivated("accounts"))
		return array (
			"PluginAccountsAccountType" => $LANG['plugin_accounts']['setup'][2],
			"PluginAccountsAccountState" => $LANG['plugin_accounts'][9]
		);
	else
		return array ();
}
...
function plugin_accounts_getAddSearchOptions($itemtype) {
   ...
   if (in_array($itemtype, PluginAccountsAccount::getTypes(true))) {
      if (plugin_accounts_haveRight("accounts","r")) {
         ...
         $sopt[1900]['name']=$LANG['plugin_accounts']['title'][1]." - ".$LANG['plugin_accounts'][7];
         $sopt[1900]['forcegroupby']=true;
         $sopt[1900]['datatype']='itemlink';
         $sopt[1900]['itemlink_type']='PluginAccountsAccount';
         if ($itemtype != 'User') {
            $sopt[1900]['joinparams']     = array('beforejoin'
                                                => array('table'      => 'glpi_plugin_accounts_accounts_items',
                                                         'joinparams' => array('jointype' => 'itemtype_item')));
         }
         $sopt[1901]['table']='glpi_plugin_accounts_accounttypes';
         $sopt[1901]['field']='name';
         $sopt[1901]['name']=$LANG['plugin_accounts']['title'][1]." - ".$LANG['plugin_accounts'][12];
         $sopt[1901]['forcegroupby']=true;
         $sopt[1901]['joinparams']     = array('beforejoin' => array(
                                                   array('table'      => 'glpi_plugin_accounts_accounts',
...
function plugin_accounts_MassiveActions($type) {
	...
	switch ($type) {
		case 'PluginAccountsAccount' :
         if ($_SESSION['glpiactiveprofile']['interface'] == 'central') {
            return array (
               // Specific one
               "plugin_accounts_install" => $LANG['plugin_accounts']['setup'][9],
               "plugin_accounts_desinstall" => $LANG['plugin_accounts']['setup'][10],
               "plugin_accounts_transfert" => $LANG['buttons'][48],
	...
	if (in_array($type, PluginAccountsAccount::getTypes(true))) {
		return array (
			"plugin_accounts_add_item" => $LANG['plugin_accounts']['setup'][18]
		);
	}
	return array ();
}
	...
	switch ($options['itemtype']) {
		case 'PluginAccountsAccount' :
			...
			switch ($options['action']) {
				...
				case "plugin_accounts_install" :
					Dropdown::showAllItems("item_item", 0, 0, -1, PluginAccountsAccount::getTypes(true));
					echo "<input type=\"submit\" name=\"massiveaction\" class=\"submit\" value=\"" . $LANG['buttons'][2] . "\" >";
					break;
				case "plugin_accounts_desinstall" :
					Dropdown::showAllItems("item_item", 0, 0, -1, PluginAccountsAccount::getTypes(true));
					echo "<input type=\"submit\" name=\"massiveaction\" class=\"submit\" value=\"" . $LANG['buttons'][2] . "\" >";
					break;
				case "plugin_accounts_transfert" :
   ...
   $account_item = new PluginAccountsAccount_Item();
	...
	switch ($data['action']) {
		...
		case "plugin_accounts_install" :
         foreach ($data["item"] as $key => $val) {
            if ($val == 1) {
               $input = array('plugin_accounts_accounts_id' => $key,
		...
		case "plugin_accounts_desinstall" :
         foreach ($data["item"] as $key => $val) {
           if ($val == 1) {
               $account_item->deleteItemByAccountsAndItem($key,$data['item_item'],$data['itemtype']);
            }
         }
			break;
		case "plugin_accounts_transfert" :
			if ($data['itemtype'] == 'PluginAccountsAccount') {
				foreach ($data["item"] as $key => $val) {
					if ($val == 1) {

                  $account->getFromDB($key);
                  $type = PluginAccountsAccountType::transfer($account->fields["plugin_accounts_accounttypes_id"],$data['entities_id']);
                  if ($type > 0) {
                     $values["id"] = $key;
...
function plugin_accounts_header_action() {
   global $LANG;
