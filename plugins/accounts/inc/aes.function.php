 ...
 */
function plugin_accounts_Cipher($input, $w) {    // main Cipher function 
...
function plugin_accounts_AddRoundKey($state, $w, $rnd, $Nb) {  // xor Round Key into state S
  for ($r=0; $r<4; $r++) {
...
function plugin_accounts_SubBytes($s, $Nb) {    // apply SBox to state S
...
function plugin_accounts_ShiftRows($s, $Nb) {    // shift row r of state S left by r bytes
  $t = array(4);
  for ($r=1; $r<4; $r++) {
...
function plugin_accounts_MixColumns($s, $Nb) {   // combine bytes of each col of state S
  for ($c=0; $c<4; $c++) {
 ...
 */
function plugin_accounts_KeyExpansion($key) {  // generate Key Schedule from Cipher Key
...
function plugin_accounts_SubWord($w) {    // apply SBox to 4-byte word w
  global $Sbox;  // PHP needs explicit declaration to access global variables!
  for ($i=0; $i<4; $i++) $w[$i] = $Sbox[$w[$i]];
  return $w;
}

function plugin_accounts_RotWord($w) {    // rotate 4-byte word w left by one byte
  $tmp = $w[0];
