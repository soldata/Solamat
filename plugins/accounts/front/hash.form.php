   ...
   if (isset($_POST["aeskeynew"]) && isset($_POST["aeskey"])){
      ...
      if (!empty ($_POST["aeskeynew"]) && !empty($_POST["aeskey"]) && !empty($hash)) {
         if ($hash <> hash ( "sha256" ,hash ( "sha256" ,$_POST["aeskey"]))) {
            Session::addMessageAfterRedirect($LANG['plugin_accounts'][22],true,ERROR);
            Html::back();
         } else {
            PluginAccountsHash::updateHash($_POST["aeskey"], $_POST["aeskeynew"],$hash_id);
            Session::addMessageAfterRedirect($LANG['plugin_accounts'][43],true);
            Html::back();
         }
      } else {
         Session::addMessageAfterRedirect($LANG['plugin_accounts'][5],true,ERROR);
         Html::back();
      }
   }
} else {
